...
class AgentProfileUpdatePage extends StatefulWidget {
  const AgentProfileUpdatePage({super.key});

  @override
  _AgentProfileUpdatePageState createState() => _AgentProfileUpdatePageState();
}
class _AgentProfileUpdatePageState extends State<AgentProfileUpdatePage> {
  ...
  String phoneNumber = 'Current Phone Number';
    ...
    String? getCurrentagentIdAsString() {
  final user = Supabase.instance.client.auth.currentUser;
  return user?.id; // Returns the user's UUID as a String or null if not logged in
}


Future<void> _updateagentInfo(String toUpdate) async {
  try {
    String? userId = getCurrentagentIdAsString();
    if (userId == null) {
    ...
    if (toUpdate == 'username') {
      data = await client.rpc('a_update_username', params: {
        'user_id': userId,
        'name': agentname, 
      });
    } else if (toUpdate == 'address') {
      data = await client.rpc('a_update_user_address', params: {
        'user_id': userId,
        'addrs': address,
      });
    } else if (toUpdate == 'number') {
      data = await client.rpc('a_update_user_number', params: {
        'user_id': userId,
        'number': phoneNumber, 
      });
    } else {
    ...
    print('Error updating agent information: $e');
  }
}
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  _showUpdateDialog(
                    ...
                    onSave: (value) {
                      setState(() {
                        agentname = value; // (Updates the agentname variable)
                      });
                      _updateagentInfo('username');
                    },
                  );
                },
